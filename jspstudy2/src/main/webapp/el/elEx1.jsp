...
today 속성값 : ${today}<br> <!-- session에 속성이 하나만 있으면 생략가능. -->
name 파라미터 값 : ${param.name}<br>
tel 변수값 : EL로 표현 못함, ${tel1} <br>
...
\${applicationScope.test} = ${applicationScope.test}<br><br>
\${today} = ${today}<br>
...
== ${test} : 영역담당 객체에 저장된 속성 중 이름이 test인 속성값을 출력
	...
	2. 1번 내용이 없으면, request 영역에 등록된 requestScope.test 속성값 리턴 속성 값 리턴
	3. 1,2번 내용이 없으면, session 영역에 등록된 sessionScope.test 속성값 리턴 속성 값 리턴
	4. 1,2,3번 내용이 없으면, application 영역에 등록된 applicationScope.test 속성값 리턴 속성 값 리턴
	5. 1,2,3,4번 내용이 없으면 공란을 리턴.
