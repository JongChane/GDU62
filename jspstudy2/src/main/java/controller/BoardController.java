...
public class BoardController extends MskimRequestMapping {
	...
	public String writeForm(HttpServletRequest request, HttpServletResponse response) {
		...
		if(boardid.equals("1")) {
			if(login == null || !login.equals("admin")) {
				request.setAttribute("msg", "관리자만 공지사항을 작성할 수 있습니다.");
	...
	public String write(HttpServletRequest request, HttpServletResponse response) {
		...
		//num :borad 테이블의 num컬럼의 값 중 최대값
		int num = dao.maxnum(); //등록된 게시글의 최대 num값
		board.setNum(++num); //최대값+1
		board.setGrp(num); 	 //grp컬럼 : 최초 게시물의 번호
	...
	public String list(HttpServletRequest request, HttpServletResponse response) {
		if(request.getParameter("boardid") != null){
			...
			request.getSession().setAttribute("pageNum","1"); //현재 페이지 번호
		}
		String boardid = (String)request.getSession().getAttribute("boardid");
		if(boardid == null) boardid="1";
		int pageNum = 1;
	...
	/*
	 	1.원글의 num 파라미터 저장 : num 원글의 게시물 번호
	...
	public String replyForm(HttpServletRequest request, HttpServletResponse response) {
		...
		Board b = dao.selectOne(num);//원글 정보
		request.setAttribute("b", b);
	...
	/*
		...
		4. 등록 성공시 : list.jsp로 페이지 이동
			   실패시 : "답변등록시 오류 발생" 메시지 출력 후 , replyForm로 페이지 이동	  
	 */
	...
	public String updateForm(HttpServletRequest request, HttpServletResponse response) {
		...
		if(boardid.equals("1")) {
			if(login==null || !login.equals("admin")) {		
			 request.setAttribute("msg", "관리자만 수정할 수 있습니다.");
	...
	/*
		...
		2. 비밀번호 검증
		 비밀번호 오류 메세지 출력 후 updateForm로 이동
		3. 수정
	...
	/*
			...
			3. 해당 게시물이 공지사항 게시물인 경우 관리자만 삭제 가능
			4. 게시물 삭제
